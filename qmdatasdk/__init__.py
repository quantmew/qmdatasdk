...
def auth_by_token(token, host=None, port=None):
    """使用 token 认证账号"""
    QMDataClient.set_auth_params(host=host, port=port, token=token)


def logout():
    """退出账号"""
    QMDataClient.instance().logout()
...
def is_auth():
    ...
    if not QMDataClient.instance():
        return False
    return not QMDataClient.instance().not_auth
...
def set_params(**params):
    ...
    """
    QMDataClient.set_request_params(**params)
