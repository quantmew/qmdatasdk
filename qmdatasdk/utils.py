...
def to_date_str(dt: Optional[Union[datetime.datetime, datetime.date, str, int, bytes]]) -> Optional[str]:
    if dt is None:
        return dt
    try:
...
def is_list(l: Any) -> bool:
    return isinstance(l, (list, tuple))


def convert_security(s: Optional[Union[str, Security, Sequence[Union[str, Security]]]]) -> Optional[Union[str, List[str]]]:
...
def convert_fields_to_str(s: Any) -> list[str]:
...
def normal_security_code(code: Any) -> Optional[Union[str, list[str]]]:
    """
...
def assert_auth(func: Callable) -> Callable:
    @wraps(func)
    def _wrapper(*args, **kwargs):
        from .client import QMDataClient
        if not QMDataClient.instance():
...
def hashable_lru(maxsize: int = 16) -> Callable:
    def hashable_cache_internal(func: Callable) -> Callable:
...
def get_security_type(security: str) -> str:
    exchange = security[-4:]
    code = security[:-5]
    if code.isdigit():
        if exchange == "XSHG":
